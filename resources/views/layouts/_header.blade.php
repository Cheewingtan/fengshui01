    ...
    <ul class="navbar-nav navbar-right">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
            <a class="dropdown-item" href="{{ route('users.show', Auth::id()) }}">{{ __('home.show_info') }}</a>
            <a class="dropdown-item" href="{{ route('users.edit', Auth::id()) }}">{{ __('home.edit_info') }}</a>
            <div class="dropdown-divider"></div>
            <a class="dropdown-item" id="logout" href="#">
            <form action="{{ route('logout') }}" method="POST">
                {{ csrf_field() }}
                <button class="btn btn-block btn-danger" type="submit" name="button">{{ __('home.sign_out') }}</button>
            </form>
            </a>
        </div>
      </li>
